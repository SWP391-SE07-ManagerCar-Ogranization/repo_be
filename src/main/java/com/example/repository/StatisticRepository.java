...
public interface StatisticRepository extends JpaRepository<UserTransaction, Integer> {
    ...
    @Query(value = "SELECT * FROM user_transaction WHERE DATE(create_at) = DATE(:date)", nativeQuery = true)
    List<UserTransaction> getTransactionByDate(@Param("date") Date date);


    @Query(value = "SELECT * FROM user_transaction WHERE YEAR(create_at) = YEAR(:date) AND MONTH(create_at) = MONTH(:date)", nativeQuery = true)
    List<UserTransaction> getTransactionByMonth(@Param("date") Date date);


    @Query(value = "SELECT * FROM user_transaction WHERE YEAR(create_at) = YEAR(:date)", nativeQuery = true)
